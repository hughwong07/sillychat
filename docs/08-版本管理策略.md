...
# 版本服务器配置
version_server:
  primary: https://version.xiaoshagua.chat
  fallback:
    - https://version-backup1.xiaoshagua.chat
    - https://version-backup2.xiaoshagua.chat
...
```kotlin
// android/app/src/main/java/chat/xiaoshagua/update/UpdateInstaller.kt
  ...
  build-windows:
    ...
    steps:
      ...
      - name: Update Version Server
        run: |
          curl -X POST https://version.xiaoshagua.chat/api/v1/release \
  ...
  build-android:
    ...
    steps:
      ...
      - name: Decode Keystore
        run: |
          echo ${{ secrets.ANDROID_KEYSTORE_BASE64 }} | base64 -d > android/app/xiaoshagua.keystore
      ...
      - name: Sign APK
        run: |
          apksigner sign \
            --ks android/app/xiaoshagua.keystore \
      ...
      - name: Sign AAB
        run: |
          jarsigner -keystore android/app/xiaoshagua.keystore \
...
```yaml
# 配置结构示例
# ~/.xiaoshagua/config/settings.yaml
_version: "2.0"           # 配置格式版本
...
# 1. 标记当前版本为问题版本
curl -X POST https://version.xiaoshagua.chat/api/v1/block \

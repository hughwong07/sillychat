...
本文档描述小傻瓜聊天工具的生活服务与支付集成设计，实现AI驱动的智能生活服务入口，集成多渠道支付能力，打造便捷的生活服务体验。
...
│  │                          核心支付层 (Core Payment)                       ││
│  │  ┌───────────────────────────────────────────────────────────────────┐  ││
│  │  │                      支付编排器 (Payment Orchestrator)             │  ││
│  │  │  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────────┐   │  ││
│  │  │  │ 路由决策  │  │ 风控检查  │  │ 限额管理  │  │ 对账/结算    │   │  ││
│  │  │  │  (Router) │  │  (Risk)   │  │ (Limit)   │  │ (Reconcile)   │   │  ││
│  │  │  └───────────┘  └───────────┘  └───────────┘  └───────────────┘   │  ││
│  │  └───────────────────────────────────────────────────────────────────┘  ││
│  │  ┌─────────────────────────────────────────────────────────────────┐    ││
│  │  │                    支付渠道适配层 (Channel Adapters)             │    ││
│  │  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌───────────┐ │    ││
│  │  │  │微信支付 │ │ 支付宝  │ │ 银行卡  │ │数字人民币│ │  Apple   │ │    ││
│  │  │  │ Adapter │ │ Adapter │ │ Adapter │ │ Adapter │ │  Pay     │ │    ││
│  │  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └───────────┘ │    ││
│  │  └─────────────────────────────────────────────────────────────────┘    ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                    │                                        │
...
enum PaymentStatus {
  ...
  AUTH_REQUIRED = 'auth_required',   // 需验证
  AUTHENTICATING = 'authenticating', // 验证中
  RISK_CHECKING = 'risk_checking',   // 风控检查
  CHANNEL_CALLING = 'channel_calling', // 渠道调用
  SUCCESS = 'success',               // 成功
  FAILED = 'failed',                 // 失败
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   Layer 5: 应用层安全         安全键盘 / 防截屏 / 防录屏 / 代码混淆            │
│   ─────────────────────────────────────────────────────────────────────────  │
│   Layer 4: 认证层安全         生物识别 / 支付密码 / 设备绑定 / 多因素认证      │
│   ─────────────────────────────────────────────────────────────────────────  │
│   Layer 3: 交易层安全         限额管理 / 风控引擎 / 异常检测 / 交易签名        │
│   ─────────────────────────────────────────────────────────────────────────  │
│   Layer 2: 传输层安全         TLS 1.3 / 证书固定 / 双向认证 / 防中间人        │
│   ─────────────────────────────────────────────────────────────────────────  │
│   Layer 1: 存储层安全         TEE/SE / 密钥分割 / 加密存储 / 安全擦除         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
  ...
  async authenticate(
    ...
    try {
      const result = await this.promptBiometric({
        ...
        subtitle: purpose,
        cancelLabel: '取消',
        allowDeviceCredential: false, // 不允许降级为设备密码
        confirmationRequired: amount > 1000 // 大额需二次确认
      });
  ...
  private rules: RiskRule[] = [
    ...
    // 规则4: 高频交易
    {
      ...
      condition: async (tx) => {
        ...
        return recentCount > 10; // 1分钟内超过10笔
      },
      action: 'challenge',
      challenge: 'captcha',
      message: '交易过于频繁'
...
class AIPriceComparison implements PriceComparisonEngine {
  ...
  private async aiAnalyzeComparisons(
    ...
    const prompt = `
      ...
      为每个选项添加aiRecommendation字段：
      - bestValue: 性价比最高
      - fastest: 物流最快
      - mostReliable: 最可靠
      - notRecommended: 不推荐（注明原因）
    `;
    ...
    bills.forEach(bill => {
      if (bill.dueDate.diff(Date.now(), 'days') <= 3) {
        reminders.push({
          type: 'bill_due',
          priority: 'high',
          title: `${bill.name}即将到期`,
          content: `金额：¥${bill.amount}，截止：${bill.dueDate}`,
          action: '立即缴费'
        });
      }
    });
    ...
    subscriptions.forEach(sub => {
      if (sub.renewalDate.diff(Date.now(), 'days') <= 7) {
        reminders.push({
          ...
          priority: 'medium',
          title: `${sub.name}即将续费`,
          content: `金额：¥${sub.amount}/月，上次使用：${sub.lastUsed}`,
          action: '管理订阅'
        });
      }
    });
    ...
    budgetStatus.forEach(budget => {
      if (budget.used / budget.limit > 0.8) {
        reminders.push({
          ...
          title: `${budget.category}预算即将超支`,
          content: `已使用 ${(budget.used / budget.limit * 100).toFixed(0)}%`,
          action: '查看详情'
        });
      }
    });
...
| 合规项 | 要求 | 实现方案 |
|--------|------|----------|
| **支付牌照** | 需持有第三方支付牌照或与持牌机构合作 | 与微信支付、支付宝等持牌机构合作 |
| **实名认证** | 用户需完成实名认证才能使用支付功能 | 接入公安/银行实名认证接口 |
| **交易限额** | 根据认证级别设置不同限额 | 未实名¥0/日，I类¥1000/日，II类¥10000/日，III类¥50000/日 |
| **反洗钱** | 大额交易报告、可疑交易监测 | 自动化AML系统，可疑交易自动上报 |
| **数据安全** | 符合《个人信息保护法》 | 数据加密、最小收集、用户授权 |
| **等保要求** | 支付系统需通过等保三级 | 每年安全测评，整改不合规项 |
| **PCI DSS** | 支付卡行业数据安全标准 | 符合PCI DSS Level 1要求 |
...
const securityNotices: Record<string, SecurityNotice> = {
  ...
  firstPayment: {
    ...
    level: 'warning',
    title: '首次支付提醒',
    message: '您首次向该商户付款，请确认商户信息',
    actions: [
      { label: '查看商户详情', action: 'view_merchant' },
      { label: '确认无误', action: 'confirm' }
    ]
  },
  ...
  largeAmount: {
    ...
    level: 'warning',
    title: '大额交易验证',
    message: '交易金额超过¥5000，需要二次验证',
    actions: [
      { label: '短信验证', action: 'sms_verify' },
      { label: '人脸验证', action: 'face_verify' }
    ]
  },
  ...
  riskDetected: {
    ...
    level: 'danger',
    title: '⚠️ 风险警告',
    message: '系统检测到该交易存在异常，建议取消',
    actions: [
      { label: '取消交易', action: 'cancel', primary: true },
      { label: '坚持支付', action: 'proceed_anyway' }
    ]
  }
};
```
...
class DeviceSecurityManager {
  ...
  async calculateTrustScore(deviceId: string, userId: string): Promise<number> {
    const factors = await Promise.all([
      this.checkDeviceAge(deviceId),           // 设备使用时长
      this.checkDeviceConsistency(deviceId, userId), // 使用一致性
      this.checkSecurityFeatures(deviceId),    // 安全功能启用
      this.checkRecentActivity(deviceId),      // 近期活动
      this.checkLocationConsistency(deviceId)  // 地点一致性
    ]);
  ...
  async freezeDevice(deviceId: string, reason: string): Promise<void> {
    ...
    await this.notifyUser(deviceId, {
      type: 'device_frozen',
      message: `您的设备因${reason}已被冻结，请联系客服解冻`,
...
| 阶段 | 时间 | 功能 | 优先级 |
|------|------|------|--------|
| **Phase 1** | 2周 | 支付核心 + 微信/支付宝 | P0 |
| **Phase 2** | 2周 | 安全体系 + 风控引擎 | P0 |
| **Phase 3** | 2周 | 生活服务API接入 | P1 |
| **Phase 4** | 2周 | AI比价 + 消费分析 | P1 |
| **Phase 5** | 2周 | 合规审计 + 数据保护 | P1 |
| **Phase 6** | 1周 | 性能优化 + 压力测试 | P2 |
